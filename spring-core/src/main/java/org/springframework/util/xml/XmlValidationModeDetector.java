...
public class XmlValidationModeDetector {
	 ...
	 * 在提供的InputStream中检测XML文档的验证模式。请注意，提供的InputStream在返回之前由此方法关闭
	 *
	...
	public int detectValidationMode(InputStream inputStream) throws IOException {
		...
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream))) {
			...
			String content;
			// <Spring分析点5-1> 循环，逐行读取 XML 文件的内容
			while ((content = reader.readLine()) != null) {
				...
				// <Spring分析点5-2> 包含 DOCTYPE 为 DTD 模式
				if (hasDoctype(content)) {
					isDtdValidated = true;
					break;
				}
				// <Spring分析点5-3>  hasOpeningTag 方法会校验，如果这一行有 < ，并且 < 后面跟着的是字母，则返回 true 。
			...
			// <Spring分析点5-4> 返回 VALIDATION_AUTO 模式
	 ...
	 * 提供的内容是否包含XML开始标记。如果解析状态当前在XML注释中，那么此方法总是返回false。在将剩余内容传递给此方法之前，所有注释标记都将被所提供的内容使用
	 */
